...
class NetconfContractTests(unittest.TestCase):
    ...
    def test_deploy_commit_check_runs_in_normal_mode_and_binds_fresh_digest(self):
        ...
        rollback_pending = self.read("scripts/rollback-pending.sh")
        rollback_playbook = self.read("playbooks/rollback-pending.yml")
        self.assertIn("rollback-pending $digest", rollback_pending)
        self.assertIn("playbooks/rollback-pending.yml", rollback_pending)
        self.assertIn("scripts/rollback-pending.sh", self.read("scripts/dispatch.sh"))
        self.assertIn("scripts/rollback-pending.sh", self.read("scripts/with-openbao-runtime.sh"))
        self.assertIn("rollback: 1", rollback_playbook)
        self.assertIn("Ansible candidate ' ~ junos_expected_digest", rollback_playbook)
        self.assertIn("Rollback failed candidate {{ junos_expected_digest }}", rollback_playbook)
        recovered_pending = self.read("scripts/confirm-recovered-pending.sh")
        confirm_playbook = self.read("playbooks/confirm.yml")
        self.assertIn("confirm-recovered $pending_digest as $candidate_digest", recovered_pending)
        self.assertIn("junos_candidate_digest", recovered_pending)
        self.assertIn("scripts/confirm-recovered-pending.sh", self.read("scripts/dispatch.sh"))
        self.assertIn("scripts/confirm-recovered-pending.sh", self.read("scripts/with-openbao-runtime.sh"))
        self.assertIn("junos_candidate_digest | default(junos_expected_digest)", confirm_playbook)
