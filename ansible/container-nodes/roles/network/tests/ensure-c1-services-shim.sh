...
case "$*" in
 ...
 '-j route show 10.25.13.64/27') [[ ${FAIL:-} != route-inspect ]] || exit 78; if [[ ${FAIL:-} == route-drift ]]; then printf '%s\n' '[{"dst":"10.25.13.0/24","dev":"c1-svc-shim","scope":"link"}]'; elif [[ -e $s/route ]]; then printf '%s\n' '[{"dst":"10.25.13.64/27","dev":"c1-svc-shim","scope":"link"}]'; else printf '[]\n'; fi;;
 'route add 10.25.13.64/27 dev c1-svc-shim') [[ ${FAIL:-} != route-add ]] || exit 79; touch "$s/route";;
 *) exit 80;; esac
SH
chmod +x "$work/ip"
