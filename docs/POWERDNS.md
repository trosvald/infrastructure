...
The canonical directory and reconciliation script are also mounted read-only under `/deployment` in
the PowerDNS container. PowerDNS does not read those paths; they make Doco-CD recreate the server
after reconciliation so it opens the atomically replaced database rather than retaining the old
SQLite inode.
