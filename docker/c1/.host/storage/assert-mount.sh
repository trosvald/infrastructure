#!/usr/bin/env bash
set -euo pipefail
[[ $# == 2 ]] || { printf 'Usage: %s <containers|librefs> <mount-path>\n' "$0" >&2; exit 64; }
readonly ROLE="$1" TARGET="$2" STATE_DIR="${STATE_DIR:-/var/lib/c1-storage}"
case "$ROLE:$TARGET" in containers:/srv/containers) label=c1_containers ;; librefs:/srv/librefs) label=c1_librefs ;; *) printf 'ERROR: unapproved mount role/path\n' >&2; exit 1 ;; esac
