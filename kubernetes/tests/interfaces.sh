...
for removed in apply-ks delete-ks sync view-secret debug-node browse-pvc; do
...
for required in validate render-root status prune-pods activate-repositories activate-controllers activate-configs activate-apps suspend resume inventory-retired-data accept-cilium audit-host-firewall enforce-host-firewall-node; do
